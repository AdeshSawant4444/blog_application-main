...
def add_blogs(request):
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            category=Category(user=user,category=category,blog=post)
            tag=Tag(user=user,tag=tag,blog=post)
            category.save()
            tag.save()
            return render(request, "add_blogs.html",{'obj':obj, 'alert':alert,'category':category,'tag':tag})
    else:
        form=BlogPostForm()
